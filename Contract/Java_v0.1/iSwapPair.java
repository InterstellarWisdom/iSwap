...
//    Event Mint(Address indexed sender, BigInteger amount0, BigInteger amount1);
    ...
    public iSwapPair(){
        super("liquitiy_token","lpt",BigInteger.valueOf(1),10);
        //等工厂合约部署后，把地址写死
        //初始化变量的值
...
//    得到储存值,初步版本时间戳未设置
    ...
    //仅前期做测试看下初试化合约成功没有
    @View
    public HashMap testToken(){
        HashMap hashMap = new HashMap();
        hashMap.put("tokenA",token0);
        hashMap.put("tokenB",token1);
        hashMap.put("totalSupply",totalSupply);
        return hashMap;
    }
    ...
    /*
        ...
        若开启feeOn开关则每笔交易的手续费有百分之6流向开发团队
        待测试
    */
    private boolean _mintFee(BigInteger _reserve0,BigInteger _reserve1){
        ...
        //这里稍微有做下调整
        if (_feeTo.equals("0")==false&&_feeTo!=null){
            feeOn = true;
        }
        ...
        if (feeOn){
            if (_kLast.compareTo(BigInteger.valueOf(0))!=0){
                ...
                if (rootk.compareTo(rootk)>0){
                    BigInteger numerator = totalSupply.multiply(rootk.subtract(rootKLast));
                    BigInteger denominator = rootk.multiply(BigInteger.valueOf(5)).add(_kLast);
    ...
    public BigInteger mint(Address to){
        Locked();
        Map<String,BigInteger> reserve = new HashMap<String, BigInteger>();
        reserve = getReserves();
        String[][] args = new String[1][];
        args[0] = new String[]{Msg.address().toString()};
        ...
        BigInteger amount1 = balance1.subtract(new BigInteger(reserve.get("reserve0").toString()));

        totalSupply = super.totalSupply();
        BigInteger liquidity = new BigInteger(String.valueOf(0));
//        boolean feeOn = _mintFee()
        if(totalSupply.compareTo(BigInteger.valueOf(1))==0){
            //流动性代币精度和其他nulsToken代币精度应该一致
            BigInteger liquidity_ = this.sqrt(amount0.multiply(amount1)).subtract(MINIMUM_LIQUIDIY);
            liquidity = new BigInteger(String.valueOf(liquidity_));
//          super.addBalance(new Address("0"),new BigInteger(String.valueOf(MINIMUM_LIQUIDIY)));
        }else{
